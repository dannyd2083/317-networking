...
public class DNSLookupService {
    ...
    private static Set<ResourceRecord> getResults(DNSNode node, int indirectionLevel) {
        ...
        //check CName, might have a CName record with the current hostname
        ...
        if (!cnameRecords.isEmpty()) {
            for (ResourceRecord cnameRecord : cnameRecords) {
                Set<ResourceRecord> cnameResult = getResults(new DNSNode(cnameRecord.getTextResult(),node.getType()),indirectionLevel+1);
                if (!cnameResult.isEmpty()) return cnameResult;
            }
        } else {
            retrieveResultsFromServer(node,rootServer);
        }
    ...
    private static void queryNextLevel(DNSNode node, Set<ResourceRecord> nameservers) {
        ...
        //already a record in cache, found the answer, return
        if (!cache.getCachedResults(node).isEmpty()) return;

        //go over name servers to get IP address
        Set <ResourceRecord> emptyNSs = new HashSet<>();
        List<ResourceRecord> rrList = new ArrayList<>(nameservers);
       for (int i = 0; i < rrList.size(); i ++) {
           ResourceRecord nameserver = rrList.get(i);
           Set<ResourceRecord> AR = cache.getCachedResults(new DNSNode(nameserver.getTextResult(),RecordType.A));
           if (!AR.isEmpty()) {
               retrieveResultsFromServer(node, AR.stream().findFirst().get().getInetResult());
               return;
           }
           else {
               emptyNSs.add(rrList.get(i));
           }
       }

       for (ResourceRecord nameserver : emptyNSs) {
           // Nameserver without additional information, call with rootserver and try to get new IP
          Set<ResourceRecord> results = getResults(new DNSNode(nameserver.getTextResult(),RecordType.A),1);
          if (!results.isEmpty()) {
              // Got a new IP, call with original nameserver for next level
              retrieveResultsFromServer(node, results.stream().findFirst().get().getInetResult());
              Set<ResourceRecord> rrResult = cache.getCachedResults(node);
              if (rrResult.isEmpty()) return;
          }
       }
    }
